...
if (httpCode == 200) {
// --- Sukses: parse response JSON dan tampilkan info presensi ---
...
Serial.println(payload);
    ...
    // Ambil field dari response, fallback ke "?" jika gagal parse
    const char* name     = !err ? (resp["data"]["name"]      | "?") : "?";
    const char* status   = !err ? (resp["data"]["status"]    | "?") : "?";
    const char* schedule = !err ? (resp["data"]["schedule"]  | "Di luar jadwal") : "?";
    const char* ts       = !err ? (resp["data"]["timestamp"] | "?") : "?";

    // Tampilkan hasil presensi di Serial Monitor
    Serial.println();
    Serial.println(F("  ╔══════════════════════════════╗"));
...
// UID tidak ditemukan di database → kartu belum didaftarkan admin
...
saveToQueue(uid); // simpan dulu, kirim nanti saat online
errorIndicator();
http.end();
return false;

} else {
// Error lain (500, 422, dll)
